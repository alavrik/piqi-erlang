...
main([NParallel]) ->
    ...
    run(Factor, 'undefined');

main([NParallel, NCycles]) ->
    Factor = list_to_integer(NParallel),
    Count = list_to_integer(NCycles),
    run(Factor, Count).
...
run(Factor, Count) ->
    ...
    test_piqi_server(Count),
    test_addressbook(Count),
    test_piqi(Count),

    ok.


test_piqi_server('undefined') ->
    test_piqi_server(_N = 100000);
test_piqi_server(N) ->
...
test_addressbook('undefined') ->
    test_addressbook(_N = 100000);
test_addressbook(N) ->
    ...
    Reader = fun addressbook_piqi:parse_address_book/2,
    Writer = fun addressbook_piqi:gen_address_book/2,
...
test_piqi(N) ->
    ...
    Reader = fun piqi_obj_piqi:parse_piqi/2,
    Writer = fun piqi_obj_piqi:gen_piqi/2,
