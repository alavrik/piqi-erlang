...
run([Filename]) ->
    ...
    PersonProtobuf = person_piqi:parse_person(Protobuf, 'pb'),
    ...
    XmlPretty = person_piqi:gen_person(Person, 'xml_pretty'),
    io:format("~n~nXML pretty: ~n~n~s~n", [XmlPretty]),
    PersonXML = person_piqi:parse_person(XmlPretty, 'xml'),
    io:format("~n~nErlang from XML: ~n~n~p~n", [PersonXML]),

    Piq = person_piqi:gen_person(Person, 'piq'),
